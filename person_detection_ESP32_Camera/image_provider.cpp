                            ...
                            int8_t *image_data)
{
  ...
  if (fb->width == 96 && fb->height == 96)
  {
    ...
    {
      image_data[i] = static_cast<int8_t>(fb->buf[i] - 128); // Chuyển từ uint8_t sang int8_t
    }
  }
  else
  {
    ...
    for (int y = 0; y < 96; y++)
    {
      for (int x = 0; x < 96; x++)
      {
        ...
        image_data[post] = static_cast<int8_t>(fb->buf[getPos] - 128); // Chuyển từ uint8_t sang int8_t
        post++;
      }
    }
  }
  ...
  // Debug Out (điều chỉnh để in dữ liệu int8_t)
  ...
  for (int y = 0; y < 96; y += 4)
  {
    ...
    {
      int getPos = y * 96 + x;
      int color = image_data[getPos] + 128; // Chuyển về khoảng [0, 255] để debug
